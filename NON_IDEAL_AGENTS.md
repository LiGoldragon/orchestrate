...
## Ambiguous worktree conclusion projects as generic partial failure

Symptom: the live daemon correctly detects a lane with multiple non-recycled
worktrees before any Jujutsu or filesystem effect, but the generated execution
boundary projects that caller rejection as `PartialApplied` with no successes and
a `System Unknown` failure instead of a dedicated conclusion-refusal reply. The
operation is fail-closed, but the presentation obscures its semantic reason from
programmatic callers.

Current workaround: treat an empty-success `PartialApplied` from lane-selected
conclusion as a refusal and leave every worktree intact. Do not infer a successful
partial teardown from that reply.

Proper fix: the same exact `WorktreeIdentity` conclusion migration should make the
ambiguous selection inexpressible. If compatibility requires retaining lane-only
conclusion until then, give the ordinary contract a typed ambiguous-selection
refusal and preserve it through generated execution rather than lowering it to a
generic failure.

## Exact typed meta maintenance requires a portable contract-family generation

Symptom: the exact `ForceRemoveRegistryRow` meta contract can be generated by
the deployed legacy schema-rust stack, but consuming its new Git revision in
Orchestrate forces Cargo to refresh the moving legacy contract dependencies.
The current `nota`/`schema-language`/`schema-rust` revisions are mutually
incompatible, so the consumer fails before generated meta bridges or runtime
handlers compile. Pinning only one dependency creates duplicate Rust trait
...
meta contract, signal contract, and daemon together. The unblock evidence is a
pushed producer pin and a clean Orchestrate contract/daemon build without local
path overrides.
